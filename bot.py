...
async def show_city(ctx: commands.Context, *, city_name=""):
    if not city_name:
        await ctx.send("Hatalı format. Lütfen şehir adını ingilizce olarak ve komuttan sonra bir boşluk bırakarak girin.")
        return
...
async def show_my_cities(ctx: commands.Context):
    cities = manager.select_ci0ties(ctx.author.id)
     # Kullanıcının kaydettiği şehirlerin listesini alma
    if cities:
